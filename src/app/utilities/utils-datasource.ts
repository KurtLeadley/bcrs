/**
 * Title: utilities/utils-datasource
 * Authors: Group 4
 * Description: bcrs
 */
...
export class SimpleDataSource<T> extends DataSource<T> {
  constructor(private row$: Observable<T[]>) {
    super();
  }
...
/**
 * Test two values to determing sort order
 *
 * @param a Value one
 * @param b Value two
 *
 * @return Value between -1 and 1 to set value one is first, second, or n/a
 */
...
function toSortFn<U>(
  ...
  return (sort$) =>
    sort$.pipe(
      map((sort) => {
        // Test active and direction empty
...
export function sortRows<U>(
  ...
  return (rows$: Observable<U[]>) =>
    combineLatest(rows$, sort$.pipe(toSortFn(sortFns, useDefault)), (rows, sortFn) => {
      if (!sortFn) {
        return rows;
      }
